...
int main() {
    ...
    BST<testStruct> *bst = new BST<testStruct>();
    ...
    for (int o = 1; o <= MAX_ORDER; o++) {
        ...
        std::cout << "Tree after inserting " << n << " elements:\n"
                  << bst->to_string(bst->root) << std::endl;
        ...
        for (int i = 0; i < m; i++) {
            testStruct so = {rand() % 10000000, ("Test"+ to_string(i))};
            BST<testStruct>::Node* result = bst->search(bst->root, so);
            if (result != nullptr) {
                hits++;
            }
        }
        ...
        std::cout << "h" << bst->calculateHeight(bst->root) << std::endl;
        std::cout << "h/log2 (n+1) " << bst->calculateHeight(bst->root) / log2(n + 1) << std::endl;
