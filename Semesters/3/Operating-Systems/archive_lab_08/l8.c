...
void runChildProcess(char *childArg, char *processType) {
    printf("%s process (pid: %d, ppid: %d): %s\n", processType, getpid(), getppid(), childArg);
    
    char *child_argv[] = {"./b", childArg, NULL};
    execv("./b", child_argv);
...
int main(int argc, char *argv[]) {
    ...
    if (length == 1) {
        ...
        printf("Returned value (parent pid: %d): %d\n", getpid(), digit);
        return digit;
    }
    ...
    printf("Parent process (pid: %d): Dividing input '%s' into upper half '%s' and lower half '%s'\n", getpid(), input, upper_half_str, lower_half_str);
    ...
    if (first_child_pid < 0) {
        ...
        if (second_child_pid < 0) {
            ...
            printf("Parent process (pid: %d): 1st child (pid: %d) terminated with code %d\n", getpid(), terminated_pid, WEXITSTATUS(status_first_child));
