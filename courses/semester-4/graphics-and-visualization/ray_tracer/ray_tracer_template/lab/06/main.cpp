...
//     PRINT_MAT3(ray_matrix);
    ...
    // ...
    glm::vec3 promien_pierwotny0 = {scene.cam.width/2-1+0.5,  scene.cam.height/2-1, 1};
    glm::vec3 promien_pierwotny1 = {0.5, 0.5, 1};
    glm::vec3 promien_pierwotny2 = {scene.cam.width/2 -1 + 0.5,0.5, 1};
    glm::vec3 promien_pierwotny3 = {0.5, scene.cam.height/2 -1 + 0.5, 1};
    glm::vec3 promien_pierwotny4 = {scene.cam.width -1 + 0.5, scene.cam.height -1 + 0.5, 1};

    ray.dir = glm::normalize(ray_matrix * promien_pierwotny0);
    std::cout << promien_pierwotny0[0] << "," << promien_pierwotny0[1] << " -> ";
    PRINT_VEC3("ray.dir: ", ray.dir);
    ray.dir = glm::normalize(ray_matrix * promien_pierwotny1);
    std::cout << promien_pierwotny1[0] << "," << promien_pierwotny1[1] << " -> ";
    PRINT_VEC3("ray.dir: ", ray.dir);
    ray.dir = glm::normalize(ray_matrix * promien_pierwotny2);
    std::cout << promien_pierwotny2[0] << "," << promien_pierwotny2[1] << " -> ";
    PRINT_VEC3("ray.dir: ", ray.dir);
    ray.dir = glm::normalize(ray_matrix * promien_pierwotny3);
    std::cout << promien_pierwotny3[0] << "," << promien_pierwotny3[1] << " -> ";
    PRINT_VEC3("ray.dir: ", ray.dir);
    ray.dir = glm::normalize(ray_matrix * promien_pierwotny4);
    std::cout << promien_pierwotny4[0] << "," << promien_pierwotny4[1] << " -> ";
    PRINT_VEC3("ray.dir: ", ray.dir);
    /// creates raster image object
    CImage image01(scene.cam.width, scene.cam.height);
    CImage image02(scene.cam.width, scene.cam.height);
    CImage image03(scene.cam.width, scene.cam.height);
    ...
    for(int j = 0; j < scene.cam.height; j++) {
        for(int i = 0; i < scene.cam.width; i++) {
            ...
            rgb.x = (ray.dir.x + 1.0f)/2.0f; // conversion from <−1,1> to <0,1> image.setPixel(i, j, rgb);
            image01.setPixel(i, j, rgb);
            rgb = {0.0f,0.0f,0.0f};

            rgb.y = (ray.dir.y + 1.0f)/2.0f;
            image02.setPixel(i, j, rgb);
            rgb = {0.0f,0.0f,0.0f};

            rgb.z = (ray.dir.z + 1.0f)/2.0f;
            image03.setPixel(i, j, rgb);
...
// computes ray.pos and ray.dir
     ...
     cv::imshow("image01", image01.getImage());
     image01.save("output01.png", false);
     cv::waitKey();

     cv::imshow("image02", image02.getImage());
     image02.save("output02.png", false);
     cv::waitKey();

     cv::imshow("image03", image03.getImage());
     image03.save("output03.png", false);
     cv::waitKey();
